...
if (isset($_POST["reg_btn"])) {
    ...
    if (empty($fsname) || empty($lsname) || empty($emails) || empty($pwd) || empty($contnumber)){
      $error = 1;
      $userExist = 0;
    }
    else{
      ...
      $userExist = 0;
        ...
        if (mysqli_num_rows($result) === 0) {    
            ...
            $_SESSION['FIRST_NAME'] = $fsname;
            $_SESSION['LAST_NAME'] = $lsname;
            $_SESSION['CONTACT_NUMBER'] = $contnumber;
...
<body>
  ...
  <script>
    if (<?php echo $error ?> == 1){
      alert("Customer Registration Failed! ☹️");
    }
    else if (<?php echo $userExist ?> == 1){
      alert("Customer Already Registration! ☹️");
    }
  </script>
</body>
</html>
